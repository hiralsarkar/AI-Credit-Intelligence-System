...
#
# Composite-PD weights. Module A and B are trained on different populations, so this
# blend cannot be fitted on a joint label; it is a documented governance choice.
# Rationale, discrimination-optimal reference, and sensitivity: see
# 05_governance/composite_weight_rationale.md (regenerated by calibrate_weights.py).
# Module A is anchored primary at origination (always available; behavioural B is an
# overlay that is absent for thin-file applicants). Re-review on any model retrain.
SIGNAL_WEIGHTS = {
    "module_a": 0.60,
    "module_b": 0.40,
}
